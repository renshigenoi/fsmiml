...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .login-logo-wrap img { height: 44px; width: auto; display: block; }
        .login-screen h1 { font-size: 24px; font-weight: 800; margin: 0 0 4px; text-align: center; }
        .login-tagline { text-align: center; color: var(--on-dark-2); font-size: 13.5px; margin: 0; }
        ...
        .header-title span { font-size: 11px; color: var(--on-dark-2); }
        ...
        .greet-band h2 { margin: 0 0 2px; font-size: 20px; font-weight: 800; }
        ...
        .scroll-x-bar::-webkit-scrollbar { display: none; }
        ...
        .wo-card:active { transform: scale(.982); border-color: var(--navy-300); }
        .wo-card .row1 { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }
        .wo-card .number { font-weight: 800; font-size: 15px; color: var(--navy-700); }
        .wo-card .cust { font-weight: 700; font-size: 14.5px; color: var(--ink); margin-bottom: 6px; }
        .wo-card .sub { font-size: 12.5px; color: var(--muted); display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
        ...
        /* =========================================================
           FLOATING PWA INSTALL SHEET (Top Floating Banner)
        ========================================================= */
        .install-sheet {
            position: fixed;
            ...
            border-radius: 18px;
            padding: 14px 16px;
            box-shadow: 0 16px 40px rgba(6, 20, 41, .28);
            border: 1px solid var(--line);
        ...
        .install-sheet .is-body { flex: 1; }
        .install-sheet .is-title { font-size: 14.5px; font-weight: 800; color: var(--ink); }
        .install-sheet .is-desc { font-size: 12px; color: var(--muted); margin-top: 2px; line-height: 1.35; }
        ...
        .detail-head {
            ...
            background: var(--brand-grad); color: var(--on-dark);
            padding: env(safe-area-inset-top, 0) 14px 0;
        }
        .detail-head-inner { display: flex; align-items: center; gap: 12px; padding: 12px 0; }
        .detail-head .d-num { font-size: 16px; font-weight: 800; }
        .detail-head .d-sub { font-size: 11.5px; color: var(--on-dark-2); }
        ...
        .detail-body { padding: 14px 16px 100px; }

        .status-banner {
            border-radius: var(--r); padding: 16px; color: #fff; margin-bottom: 14px;
        }
        .status-banner .s-label { font-size: 17px; font-weight: 800; }
        .status-banner .s-hint { opacity: .9; font-size: 12.5px; margin-top: 4px; }

        .card {
            background: var(--surface); border-radius: var(--r);
            border: 1px solid var(--line); padding: 16px; margin-bottom: 12px;
        }
        .card h4 { margin: 0 0 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; color: var(--muted); }
        .kv { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--bg-2); font-size: 14px; }
        .kv:last-child { border-bottom: 0; padding-bottom: 0; }
        .kv .k { color: var(--muted); }
        .kv .v { text-align: right; font-weight: 600; color: var(--ink-2); }

        #detail-map { height: 180px; border-radius: var(--r-sm); border: 1px solid var(--line); margin-bottom: 10px; }

        .btn-maps-action {
            display: flex; align-items: center; justify-content: center; gap: 8px;
            width: 100%; border: 0; border-radius: var(--r-sm); padding: 12px;
        ...
        .action-btn {
            display: flex; align-items: center; justify-content: center; gap: 8px;
            width: 100%; border: 0; border-radius: var(--r-sm); padding: 15px;
        ...
        .action-btn.blue { background: linear-gradient(135deg, #0ea5e9, #0284c7); }
        .action-btn.amber { background: linear-gradient(135deg, #f59e0b, #d97706); }
        .action-btn.ghost { background: transparent; color: var(--red-500); border: 1.5px solid rgba(200, 16, 46, .25); }
        ...
        .nav-item {
            ...
            color: var(--muted); font-size: 11px; font-weight: 600; cursor: pointer; flex: 1;
        }
        .nav-item.active { color: var(--red-500); font-weight: 800; }
        .nav-item .nav-icon { font-size: 19px; }

        .toast {
            position: fixed; top: calc(env(safe-area-inset-top, 12px) + 12px); left: 50%;
            transform: translateX(-50%); z-index: 100; background: var(--navy-800);
            color: #fff; padding: 12px 20px; border-radius: var(--r-sm); font-size: 13.5px;
            box-shadow: var(--shadow-lg);
        }
        ...
        ========================================================= */
        .modal-backdrop {
            position: fixed; inset: 0; background: rgba(6, 20, 41, .65); z-index: 90;
        ...
        .modal-grip { width: 40px; height: 4.5px; background: var(--line-2); border-radius: 99px; margin: 0 auto 16px; }
        ...
        .modal .desc { color: var(--muted); font-size: 12.5px; margin: 2px 0 0; }
        ...
        .eye-toggle:active { opacity: 1; background: var(--bg-2); }
        ...
        .chk-item.valid { color: var(--green); font-weight: 700; }
...
<body>
    ...
    @verbatim
        <div id="app" v-cloak>
            ...
            <div v-if="showInstallBanner" class="install-sheet">
                <button type="button" class="is-close-btn" @click="dismissInstall" title="Tutup">✕</button>
                <div class="is-icon">📲</div>
                <div class="is-body">
                    <div class="is-title">Pasang FSM Teknisi</div>
                    <div class="is-desc" v-if="installEvent">
                        Akses lebih cepat &amp; pengalaman aplikasi penuh.
                    </div>
                    <div class="is-desc" v-else>
                        Tekan menu <strong>⋮</strong> / <strong>Share</strong> browser → pilih <strong>"Tambahkan ke Layar Utama"</strong>.
                    </div>
                    <div class="is-actions">
                        <button v-if="installEvent" type="button" class="is-btn" @click="installApp">Pasang Sekarang</button>
                        <button type="button" class="is-later" @click="dismissInstall">Nanti (10d)</button>
                    </div>
                </div>
            </div>
                ...
                <div class="app-main-content">
                    ...
                    <div v-if="view === 'home'">
                        ...
                        <div class="scroll-x-bar">
                            <button class="filter-chip" :class="{ active: subFilter === 'all' }" @click="subFilter = 'all'">
                                Semua
                            </button>
                            <template v-if="activeTab === 'processing'">
                                <button class="filter-chip" :class="{ active: subFilter === 'waiting_acceptance' }" @click="subFilter = 'waiting_acceptance'">
                                    Konfirmasi ({{ pendingOrders.length }})
                                </button>
                                <button class="filter-chip" :class="{ active: subFilter === 'on_the_way' }" @click="subFilter = 'on_the_way'">
                                    Perjalanan
                                </button>
                                <button class="filter-chip" :class="{ active: subFilter === 'installation' }" @click="subFilter = 'installation'">
                                    Pemasangan
                                </button>
                            </template>
                            <template v-else>
                                <button class="filter-chip" :class="{ active: subFilter === 'finished' }" @click="subFilter = 'finished'">
                                    Selesai
                                </button>
                                <button class="filter-chip" :class="{ active: subFilter === 'rejected' }" @click="subFilter = 'rejected'">
                                    Ditolak
                                </button>
                            </template>
                        </div>
                        ...
                        <template v-else>
                            ...
                            <div v-if="activeTab === 'processing'" class="section">
                                ...
                                <button v-for="wo in filteredProcessingOrders" :key="wo.id" class="wo-card" @click="openDetail(wo)">
                                    <div class="row1">
                            ...
                            <div v-if="activeTab === 'history'" class="section">
                                <div class="search-bar">
                                    ...
                                    <input type="text" v-model.trim="searchQuery" placeholder="Cari SPK / customer..." autocomplete="off">
                                ...
                                <button v-for="wo in filteredHistoryOrders" :key="wo.id" class="wo-card" @click="openDetail(wo)" style="opacity:.88;">
                                    <div class="row1">
                                        <span class="number">{{ wo.number }}</span>
                        ...
                        <div class="detail-body">
                            ...
                            <div class="card">
                                ...
                                <div v-if="current.service_location && current.service_location.latitude">
                                    ...
                                    <a class="btn-maps-action" :href="getNavigationUrl(current.service_location)" target="_blank" rel="noopener">
                ...
                <div class="modal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="pass-input-box">
                            ...
                            <input :type="showPassCurrent ? 'text' : 'password'" v-model.trim="passModal.current" placeholder="Masukkan password lama">
                    ...
                    <div class="form-group">
                        ...
                        <div class="pass-input-box">
                            <span class="field-ico">🔑</span>
                            <input :type="showPassNext ? 'text' : 'password'" v-model.trim="passModal.next" placeholder="Minimal 6 karakter">
                    ...
                    <div class="form-group">
                        ...
                        <div class="pass-input-box">
                            <span class="field-ico">✨</span>
                            <input :type="showPassConfirm ? 'text' : 'password'" v-model.trim="passModal.confirm" placeholder="Ketik ulang password baru">
                    ...
                    <div v-if="passModal.error" style="background:var(--red-100); color:var(--red-700); padding:10px 12px; border-radius:var(--r-sm); font-size:12.5px; margin-bottom:12px; border:1px solid #fecdd3;">
                        ⚠️ {{ passModal.error }}
                    </div>
                    ...
                    <div class="modal-actions">
                        ...
                        <button class="ok-red" type="button" 
                                :disabled="busy || passModal.next.length < 6 || passModal.next !== passModal.confirm || !passModal.current" 
        ...
        <script>
            ...
            const app = Vue.createApp({
                ...
                computed: {
                    todayLabel() {
                        return new Date().toLocaleDateString('id-ID', { weekday: 'short', day: 'numeric', month: 'short' });
                    },
                    firstName() {
                    ...
                    activeOrders() {
                        return this.orders.filter(function(wo) {
                            const a = this.myAssignment(wo);
                            return a && (a.status === 'accepted' || a.status === 'pending' || ['on_the_way', 'arrived', 'installation'].includes(wo.status));
                        }.bind(this));
                    },
                    historyOrders() {
                        return this.orders.filter(function(wo) {
                            const a = this.myAssignment(wo);
                            if (!a) return false;
                            return ['rejected', 'superseded'].includes(a.status) || ['finished', 'cancelled', 'failed'].includes(wo.status);
                        }.bind(this));
                    },
                    ...
                    filteredHistoryOrders() {
                        ...
                        return this.historyOrders.filter(function(wo) {
                            ...
                            if (q) {
                                const num = String(wo.number || '').toLowerCase();
                    ...
                    bannerStyle() {
                        ...
                        return { background: 'linear-gradient(135deg, ' + meta.color + 'ee, ' + meta.color + '99)' };
                    },
                ...
                methods: {
                    initInstallBanner() {
                        // Jangan tampilkan jika sudah dijalankan dalam mode PWA / Standalone
                        const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone;
                        if (isStandalone) return;

                        // Periksa apakah user pernah memilih "Nanti" (dismiss)
                        if (sessionStorage.getItem('fsm_pwa_dismissed') === '1') return;

                        const triggerShow = (evt) => {
                            if (evt) this.installEvent = evt;
                            this.showInstallBanner = true;

                            // Hapus auto-hide timer lama jika ada
                            clearTimeout(this.installTimer);
                            // Setel auto-hide persis 10 detik setelah banner tampil
                        ...
                        if (globalInstallPrompt) {
                            triggerShow(globalInstallPrompt);
                        } else {
                            window.addEventListener('beforeinstallprompt', (e) => {
                                triggerShow(e);
                            });

                            // Fallback jika browser mobile/iOS tidak mendukung beforeinstallprompt
                            const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
                            if (isMobile) {
                                setTimeout(() => {
                                    if (!this.showInstallBanner && !sessionStorage.getItem('fsm_pwa_dismissed')) {
                                        triggerShow(null);
                                    }
                                }, 1500);
                            }
                    ...
                    myAssignment(wo) {
                        ...
                        return wo.assignments.find(function(a) { return a.technician_id === techId; }) || null;
                    },
                    ...
                    fmtDate(val) { return val ? new Date(val).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'; },
                    fmtDateTime(val) {
                        if (!val) return '-';
                    ...
                    async api(path, options = {}) {
                        ...
                        if (res.status === 401) { this.forceLogout(); throw new Error('Sesi berakhir.'); }
                    ...
                    openDetail(wo) { this.view = 'detail'; this.current = wo; },
                    goHome() { this.view = 'home'; this.current = null; },
                    forceLogout() {
                        this.token = ''; this.user = null; this.view = 'home';
                        localStorage.removeItem('fsm_tech_token');
                        localStorage.removeItem('fsm_tech_user');
                    },
                    async doLogout() {
                        try { await this.api('/auth/logout', { method: 'DELETE' }); } catch (err) {}
                        this.forceLogout();
                    },
                    openPassModal() {
